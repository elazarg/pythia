...
class SimpleQmpClient:
    ...
    async def __aenter__(self):
        try:
            ...
            ex.add_note(f"Check that the VM is running and listens at port {self.port}.")
            raise

    async def __aexit__(self, exc_type, exc_val, exc_tb):
        await self.qmp.disconnect()

    async def execute(self, cmd: str, args: Optional[Mapping[str, object]] = None) -> dict:
    ...
    async def dump(self, filename) -> None:
        res = await self.execute('dump-guest-memory', {'paging': False, 'protocol': f'file:{filename}'})
        if res:
